...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case AppTab.PLANNER:
        return <PlannerPanel profile={profile} documents={documents} />;
      case AppTab.SUMMARY:
        return <SummaryPanel profile={profile} documents={documents} />;
      default:
        return null;
    }
  };
  ...
  return (
    <div
      className={`flex h-screen overflow-hidden transition-colors duration-300 ${isDarkMode ? "bg-[#0a0a0c] text-slate-100" : "bg-white text-slate-900"}`}
    >
      <Sidebar
        activeTab={activeTab}
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden pb-[64px] md:pb-0 transition-all duration-300">
        ...
        <div
          className={`flex-1 overflow-y-auto custom-scrollbar ${isChatTab ? "p-0" : "p-3 md:p-6 bg-white dark:bg-[#0a0a0c] transition-colors"}`}
        >
          <div
            className={`h-full ${isChatTab ? "max-w-none" : "max-w-6xl mx-auto"}`}
          >
            {renderContent()}
          </div>
        </div>
          ...
          <div className="relative w-full max-w-4xl bg-white dark:bg-[#0f1115] rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="flex items-center justify-between p-4 md:p-6 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm md:text-lg font-black text-slate-800 dark:text-white uppercase tracking-tight">
                    Hướng dẫn sử dụng AI Study Copilot
                  </h3>
                  <p className="text-[10px] md:text-xs text-slate-500 dark:text-slate-400 font-bold">
                    Làm chủ ứng dụng trong 2 phút cùng Copilot
                  </p>
                </div>
              </div>
              <button
